 ...
 * これより暗いパスは予報に載せない[等級]。
 *
 * 肉眼の限界は暗い空で6等だが、それは瞳孔が完全に開いた状態で
 * どこを見ればよいか分かっている場合の話。
 * 市街地で動く点を探すなら 4.5等が現実的な限界で、
 * それ以上暗いものを「見えます」と案内しても徒労に終わる。
 *
 * 予報JSONの肥大化を抑える効果もある（BlueBirdは12機あるため）。
 */
export const MAX_NAKED_EYE_MAGNITUDE = 4.5;

/**
 * 予報に残す「軌道条件だけの見やすさ」の下限（0〜1）。
 *
 * BlueBirdを12機追跡すると1地点あたり40件前後のパスが出るが、
 * その8割は仰角が低い・暗い・深夜といった理由で誰も見に行かない。
 * 情報として役に立たないうえ、予報JSONを10倍以上に膨らませる。
 *
 * 天気ではなく軌道条件で切るのが要点。
 * 曇り予報の日に「価値なし」と消してしまうと、晴れたときに何も出せなくなる。
 */
export const MIN_STORED_GEOMETRIC_QUALITY = 0.18;

/**
 * 上の基準で全部消えてしまった場合でも、これだけは残す件数。
 *
 * 「次のチャンスは◯日」という案内は、条件が悪いパスしかない時期こそ必要になる。
 */
export const MIN_STORED_PASSES = 8;
